...
- A `.gmd` is an XML-like plist wrapper. `k4` contains canonical padded URL-safe Base64 of gzip-compressed level text. Keep the encoder's terminal `=` characters; never strip or silently reconstruct them during validation. The inner text is a header followed by semicolon-separated objects; each object is comma-separated key/value pairs. Human exports can contain opaque nonnumeric keys, so preserve unknown tokens.
...
Key `24` is an enumerated Z layer, not an arbitrary signed depth. Conservative observed values are `-5,-3,-1,1,3,5,7,9,11,13`; use target-version donor records as authority. Put fine ordering in key `25`. A plain/empty editor after an apparently successful import is a fatal compatibility failure: the outer plist may be accepted while Geometry Dash rejects `k4` or the decoded level string. Inspect the serialized `k4` alphabet, `len % 4`, and terminal padding first; then compare raw prologue, line breaks, outer key/tag order, local/online identity, `k34`, and template-only metadata before bisecting inner changes. Never trust generic XML validity, a permissive decoder, or a semantic decode/re-encode test alone.
